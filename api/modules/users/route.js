...
module.exports = (module) => {
  ...
  module.router.post('/login', (req, res, next) => {
    global.helpers.database.login(req, res, module.model).then( (result) => {
      if(!result.data.enabled) {
        next(module.lib.httpError(404, 'El usuario no esta habilitado'));
      } 
      else {
        res.send(result);
      }
   ...
   * @return {void}
   */
  module.router.get('/recoveryPassword/username/:username', (req, res, next) => {
    global.helpers.database.recoveryPassword({username:req.params.username}, module.model)
      .then(result => res.send(result))
      .catch(next);
  });
